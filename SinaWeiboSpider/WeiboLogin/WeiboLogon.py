...
import binascii
import urllib.parse
from SinaWeiboSpider.WeiboLogin import WeiboCertify
...
class WeiBoLogin(object):
    ...
    def login(self, user_name, pass_word, proxies=None):
        ...
        self.cookie_jar, self.opener = WeiboCertify.make_cookiejar_opener(is_cookie=True, proxies=proxies)
        self.opener.addheaders = WeiboCertify.make_headers(
            user_agent="pc",
            host="weibo.com",
        ...
        json_data_1 = json.loads(WeiboCertify.get_html_content(self.opener.open(login_url_1, data=WeiboCertify.make_post_data(post_dict))))
        if json_data_1["retcode"] == "0":
            ...
            html_data = WeiboCertify.get_html_content(self.opener.open(login_url_2), charset="gbk")
    ...
    def get_json_data(self, su_value):
        ...
        try:
            ...
            data = WeiboCertify.get_html_content(response, charset="utf-8")
...
if __name__ == '__main__':
    ...
    # 测试登录,输入微博的用户名和密码
    weibo = WeiBoLogin()
    weibo.login(input("请输入用户名:"), input("请输入密码:"))